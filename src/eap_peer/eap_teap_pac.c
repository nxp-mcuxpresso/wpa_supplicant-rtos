...
/* Text data format */
static const char *pac_file_hdr = "wpa_supplicant EAP-TEAP PAC file - version 1";
...
static void eap_teap_remove_pac(
    struct eap_teap_pac **pac_root, struct eap_teap_pac **pac_current, const u8 *a_id, size_t a_id_len, u16 pac_type)
{
...
static int eap_teap_copy_buf(u8 **dst, size_t *dst_len, const u8 *src, size_t src_len)
{
    if (src)
    {
...
int eap_teap_add_pac(struct eap_teap_pac **pac_root, struct eap_teap_pac **pac_current, struct eap_teap_pac *entry)
{
    ...
    eap_teap_remove_pac(pac_root, pac_current, entry->a_id, entry->a_id_len, entry->pac_type);
    ...
    if (eap_teap_copy_buf(&pac->pac_opaque, &pac->pac_opaque_len, entry->pac_opaque, entry->pac_opaque_len) < 0 ||
        eap_teap_copy_buf(&pac->pac_info, &pac->pac_info_len, entry->pac_info, entry->pac_info_len) < 0 ||
        eap_teap_copy_buf(&pac->a_id, &pac->a_id_len, entry->a_id, entry->a_id_len) < 0 ||
        eap_teap_copy_buf(&pac->i_id, &pac->i_id_len, entry->i_id, entry->i_id_len) < 0 ||
        eap_teap_copy_buf(&pac->a_id_info, &pac->a_id_info_len, entry->a_id_info, entry->a_id_info_len) < 0)
    {
        eap_teap_free_pac(pac);
        return -1;
    }
...
static int eap_teap_init_pac_data(struct eap_sm *sm, const char *pac_file, struct eap_teap_read_ctx *rc)
{
    ...
    if (os_strncmp(pac_file, "blob://", 7) == 0)
    {
        ...
        if (!blob)
        {
            wpa_printf(MSG_INFO, "EAP-TEAP: No PAC blob '%s' - assume no PAC entries have been provisioned",
                       pac_file + 7);
            os_free(rc->buf);
            return -1;
        }
    ...
    else
    {
        ...
        if (!rc->f)
        {
            wpa_printf(MSG_INFO, "EAP-TEAP: No PAC file '%s' - assume no PAC entries have been provisioned", pac_file);
            os_free(rc->buf);
            return -1;
        }
    }
...
static const char *eap_teap_parse_end(struct eap_teap_pac **pac_root, struct eap_teap_pac **pac)
{
    if (!(*pac))
...
static const char *eap_teap_parse_pac_type(struct eap_teap_pac *pac, char *pos)
{
    if (!pos)
...
static const char *eap_teap_parse_pac_opaque(struct eap_teap_pac *pac, char *pos)
{
    os_free(pac->pac_opaque);
...
static const char *eap_teap_parse_a_id_info(struct eap_teap_pac *pac, char *pos)
{
    os_free(pac->a_id_info);
...
int eap_teap_load_pac(struct eap_sm *sm, struct eap_teap_pac **pac_root, const char *pac_file)
{
    ...
    if (err)
    {
        wpa_printf(MSG_INFO, "EAP-TEAP: %s in '%s:%d'", err, pac_file, rc.line);
        return -1;
    }

    wpa_printf(MSG_DEBUG, "EAP-TEAP: Read %d PAC entries from '%s'", count, pac_file);

    return 0;
}

static void eap_teap_write(
...
static int eap_teap_write_pac(struct eap_sm *sm, const char *pac_file, char *buf, size_t len)
{
    if (os_strncmp(pac_file, "blob://", 7) == 0)
    {
    ...
    else
    {
        ...
        if (!f)
        {
            wpa_printf(MSG_INFO, "EAP-TEAP: Failed to open PAC file '%s' for writing", pac_file);
            return -1;
        }
        if (fwrite(buf, 1, len, f) != len)
        {
            wpa_printf(MSG_INFO, "EAP-TEAP: Failed to write all PACs into '%s'", pac_file);
            fclose(f);
            return -1;
        }
        os_free(buf);
...
static int eap_teap_add_pac_data(struct eap_teap_pac *pac, char **buf, char **pos, size_t *buf_len)
{
    int ret;
    ...
    *pos += ret;
    eap_teap_write(buf, pos, buf_len, "PAC-Key", pac->pac_key, EAP_TEAP_PAC_KEY_LEN, 0);
    eap_teap_write(buf, pos, buf_len, "PAC-Opaque", pac->pac_opaque, pac->pac_opaque_len, 0);
    eap_teap_write(buf, pos, buf_len, "PAC-Info", pac->pac_info, pac->pac_info_len, 0);
    eap_teap_write(buf, pos, buf_len, "A-ID", pac->a_id, pac->a_id_len, 0);
    eap_teap_write(buf, pos, buf_len, "I-ID", pac->i_id, pac->i_id_len, 1);
    eap_teap_write(buf, pos, buf_len, "A-ID-Info", pac->a_id_info, pac->a_id_info_len, 1);
    if (!(*buf))
    {
...
int eap_teap_save_pac(struct eap_sm *sm, struct eap_teap_pac *pac_root, const char *pac_file)
{
    ...
    wpa_printf(MSG_DEBUG, "EAP-TEAP: Wrote %d PAC entries into '%s'", count, pac_file);
...
int eap_teap_load_pac_bin(struct eap_sm *sm, struct eap_teap_pac **pac_root, const char *pac_file)
{
    ...
    *pac_root = NULL;
    ...
    {
        wpa_printf(MSG_INFO, "EAP-TEAP: Invalid PAC file '%s' (bin)", pac_file);
        if (!blob)
            os_free(buf);
        return -1;
    }
    ...
    wpa_printf(MSG_DEBUG, "EAP-TEAP: Read %lu PAC entries from '%s' (bin)", (unsigned long)count, pac_file);

    return 0;

parse_fail:
    wpa_printf(MSG_INFO, "EAP-TEAP: Failed to parse PAC file '%s' (bin)", pac_file);
    if (!blob)
        os_free(buf);
    if (pac)
        eap_teap_free_pac(pac);
...
int eap_teap_save_pac_bin(struct eap_sm *sm, struct eap_teap_pac *pac_root, const char *pac_file)
{
    ...
    while (pac)
    {
        if (pac->pac_opaque_len > 65535 || pac->pac_info_len > 65535)
            return -1;
        len += 2 + EAP_TEAP_PAC_KEY_LEN + 2 + pac->pac_opaque_len + 2 + pac->pac_info_len;
        pac = pac->next;
    }
    ...
    wpa_printf(MSG_DEBUG, "EAP-TEAP: Wrote %lu PAC entries into '%s' (bin)", (unsigned long)count, pac_file);

    return 0;
}

#endif /* CONFIG_EAP_TEAP */
